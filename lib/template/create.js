...
function getTpl(type) {
    ...
    var basePath = pro.findDir();

    if (basePath) {
        basePath = pro.getConfFile(basePath, DIR_TPL);
    }

    var tplPath = findTpl(type, basePath);

    if (!tplPath) {
        tplPath = findTpl(
            type, 
            require('../edp').getUserModuleDir() + '/' + DIR_TPL
        );
    }
